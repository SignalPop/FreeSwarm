...
def _read_shard_odirect_parallel(path: str, workers: int, chunk: int) -> mmap.mmap:
    ...
    if _sys.platform == "win32":
        # Windows: no O_DIRECT/preadv. Try FILE_FLAG_NO_BUFFERING first -- a buffered read of
        # a multi-GiB shard also leaves that shard in the system file cache, and across a
        # whole checkpoint that cache growth (on top of the non-reclaimable pinned banks) is
        # what makes the box unresponsive mid-load. Buffered sequential read is the fallback.
        from freetoken.utils import winio

        size = os.path.getsize(path)
        asize = ((size + _ODIRECT_BLK - 1) // _ODIRECT_BLK) * _ODIRECT_BLK
        buf = mmap.mmap(-1, asize)
        if winio.read_into(path, buf, chunk=chunk) is not None:
            return buf
        mv = memoryview(buf)
        with open(path, "rb", buffering=0) as f:
            ...
            while off < size:
                ...
                if not n:
                    break
                off += n
        del mv  # release the export so the mmap can be resized/closed by its owner
        return buf
    size = os.path.getsize(path)
    asize = ((size + _ODIRECT_BLK - 1) // _ODIRECT_BLK) * _ODIRECT_BLK
    buf = mmap.mmap(-1, asize)
...
def _ram_capped_prefetch(requested: int, shard_sizes: list[int]) -> int:
    """Shrink the prefetch depth until the reader's in-flight shard buffers fit in RAM.

    Those buffers are anonymous (non-reclaimable) and sit on top of the banks being filled,
    which are themselves ~the checkpoint size. At the default depth a checkpoint with 4 GiB
    shards keeps ~16 GiB of transient alive; on a box where the banks already dominate RAM
    that transient is the difference between a slow load and an unusable machine. Unknown
    RAM size or unmeasurable shards -> keep the requested depth.
    """
    from freetoken.utils import hostmem

    if requested <= 0 or not shard_sizes:
        return requested
    avail = hostmem.available()
    if avail is None:
        return requested
    # The banks land in the same RAM, so only what is left over may hold shard buffers.
    spare = avail - sum(shard_sizes)
    largest = max(shard_sizes)
    if largest <= 0:
        return requested
    # +2: one shard in the reader, one held by the consumer, on top of the queue's depth.
    allowed = int(spare // largest) - 2
...
def iter_expert_tensors_parallel(
    ...
    *,
    ...
    Peak host memory is ~(prefetch+2) shards + the banks the caller fills -- these are
    anonymous buffers, so unlike the serial path's file mmaps the OS cannot reclaim them
    under pressure. ``prefetch`` is therefore capped to what free RAM actually leaves for
    the transient after the banks (:func:`_ram_capped_prefetch`); depth 0 still overlaps one
    shard's read with the previous shard's placement. Order is shard-then-header order (NOT
    global), so the consumer must place by ``name``.
    """
    ...
    prefetch = _ram_capped_prefetch(
        prefetch, [os.path.getsize(os.path.join(model_path, s)) for s in shard_list]
    )
