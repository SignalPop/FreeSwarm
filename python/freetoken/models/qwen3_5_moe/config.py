...
def _shared_expert_ignored(hf_config: Any) -> bool:
    """True iff the quant config's ``ignore`` list positively covers the MoE
    ``shared_expert`` -- i.e. an 'experts-only' NVFP4 community quant that leaves the
    shared_expert (and typically attn/lm_head) bf16 while quantizing only the routed
    experts. Conservative: returns False unless shared_expert is explicitly ignored, so
    uniformly-quantized modelopt checkpoints keep their native-FP4 dense path unchanged."""
    get = _quant_accessor(hf_config)
    if get is None:
        return False
    ignore = get("ignore") or get("modules_to_not_convert") or []
    return isinstance(ignore, (list, tuple)) and any(
        "shared_expert" in str(x) for x in ignore
    )
...
def parse_config(hf_config: Any) -> ModelConfig:
    ...
    # 'Experts-only' NVFP4 community quants (e.g. abliterated MoEs) keep the shared_expert
    # bf16 -- it sits in the quant `ignore` list -- even though the routed experts are NVFP4.
    # Forcing the native-FP4 dense path there fails at load (no shared_expert weight_scale),
    # so fall back to bf16 for the shared_expert when it is explicitly ignored.
    if dense_quant == "nvfp4" and _shared_expert_ignored(hf_config):
        dense_quant = "none"
