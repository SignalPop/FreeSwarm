...
if _sys.platform == "win32":
    # tvm-ffi's Windows JIT link omits the CUDA runtime; add cudart.lib + its lib dir so
    # kernel DLLs resolve cudaLaunchKernel/registerFatBinary/etc. (Linux gets -lcudart).
    _cuda_home = os.environ.get("CUDA_PATH") or os.environ.get("CUDA_HOME") or ""
    DEFAULT_LDFLAGS = ["cudart.lib"] + (['/LIBPATH:"' + os.path.join(_cuda_home, "lib", "x64") + '"'] if _cuda_home else [])
else:
    DEFAULT_LDFLAGS = []
