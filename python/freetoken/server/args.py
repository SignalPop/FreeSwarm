...
class ServerArgs(SchedulerConfig):
    ...
    def zmq_frontend_addr(self) -> str:
        return zmq_endpoint(3, self._unique_suffix)

    @property
    def zmq_tokenizer_addr(self) -> str:
        if self.share_tokenizer:
            return self.zmq_detokenizer_addr
        result = zmq_endpoint(4, self._unique_suffix)
...
def parse_args(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Cap, in GiB, on the host RAM the MoE expert banks may hold. The banks are "
            "page-locked for the process lifetime and cannot be paged out, so a load that "
            "exceeds this is refused up front (with the numbers) instead of making the "
            "machine unresponsive. Default: 75%% of physical RAM, and never more than "
            "total minus 8 GiB. Equivalent to FREETOKEN_HOST_RAM_GB."
        ),
    )
    ...
    # --host-ram-gb is not a ServerArgs field: it is published to the environment so the
    # budget is visible to freetoken.utils.hostmem in this process AND in the engine worker
    # subprocesses, which are where the expert banks are actually allocated.
    host_ram_gb = kwargs.pop("host_ram_gb", None)
    if host_ram_gb is not None:
        os.environ["FREETOKEN_HOST_RAM_GB"] = str(host_ram_gb)
