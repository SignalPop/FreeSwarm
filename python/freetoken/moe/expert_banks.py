...
def _shard_sizes(model_path: str) -> list[int]:
    """Byte sizes of the checkpoint's safetensors shards; ``[]`` if they cannot be listed."""
    ...
    except Exception:  # noqa: BLE001 -- best-effort sizing, never fatal
        return []
    try:
        return [os.path.getsize(p) for p in glob.glob(os.path.join(model_path, "*.safetensors"))]
    except OSError:
        return []


def _expert_bank_bytes(model_path: str) -> int:
    """Bytes of MoE expert weights -- what actually gets page-locked into banks.

    Read from the safetensors headers only. Must match the control plane's own estimate
    (ui/backend/app/engine.py::_expert_bytes): the two checks run in different processes, and
    when this one counted the WHOLE checkpoint while that one counted experts, a model the
    console approved (Qwen3.6: 61.5 GiB of experts) was then refused here as 67.0 GiB --
    over a 66.5 GiB budget by the ~5.5 GiB of non-expert weights that go to VRAM, not RAM.

    Falls back to the whole checkpoint when the headers cannot be read, which over-estimates
    (safe direction). Returns 0 when there is nothing to measure.
    """
    import json
    import struct
    ...
    for shard in shards:
        try:
            ...
            return sum(_shard_sizes(model_path))
        for name, meta in header.items():
            ...
            if ".experts." in name or ".mlp.expert" in name:
                experts += end - begin
    # No expert tensors found (FTW layout, unusual naming): fall back to the checkpoint size
    # rather than reporting 0 and waving the load through.
    return experts or sum(_shard_sizes(model_path))
...
def _host_ram_fits_parallel(model_path: str) -> bool:
    ...
    extra (non-reclaimable) whole-shard buffer? Unknown -> True, i.e. keep the fast path.
    Banks ~= checkpoint size (experts dominate); transient ~= the largest shard.

    The probe is :mod:`freetoken.utils.hostmem`, which reports available RAM on Windows and
    macOS too. It previously read ``/proc/meminfo`` directly, so on Windows it always
    answered "plenty" and this fallback could never fire -- the parallel reader then took
    the memory-hungry path on exactly the boxes that could least afford it."""
    from freetoken.utils import hostmem

    avail = hostmem.available()
    if avail is None:
        return True
    sizes = _shard_sizes(model_path)
    if not sizes:
        return True
    return avail > sum(sizes) + max(sizes)


def _check_host_ram_budget(model_path: str, dummy: bool) -> None:
    """Refuse a load whose host banks would exceed the configured host-RAM budget.

    The banks are page-locked for the process lifetime, so oversubscribing host RAM does not
    degrade -- it makes the whole machine unusable, with no way out but killing the engine.
    Failing here with the numbers and the override is strictly kinder than that. The estimate
    is the EXPERT bytes (see _expert_bank_bytes), which must agree with the control plane's
    preflight; an unmeasurable checkpoint or unknown RAM size skips the check.
    """
    if dummy:
        return
    from freetoken.utils import hostmem
    ...
    if budget is None:
        return
    # Experts only: those are what get pinned. The non-expert weights go to VRAM, so counting
    # the whole checkpoint here over-stated the need and refused models that fit.
    estimate = _expert_bank_bytes(model_path)
    if estimate <= 0:
        return
    if estimate <= budget:
        logger.info_rank0(
            f"expert banks: ~{hostmem.fmt(estimate)} of host RAM "
            f"(budget {hostmem.fmt(budget)}, {hostmem.describe()})"
        )
        return
    raise RuntimeError(
        f"MoE expert banks need ~{hostmem.fmt(estimate)} of page-locked host RAM but the "
        f"budget is {hostmem.fmt(budget)} ({hostmem.describe()}).\n"
        "Page-locked banks cannot be paged out, so loading anyway would make the machine "
        "unresponsive rather than merely slow. Options:\n"
        "  - serve a smaller or quantized checkpoint (FP8/NVFP4 experts roughly halve or "
        "quarter the bank footprint), or convert to FTW;\n"
        "  - raise the cap with FREETOKEN_HOST_RAM_GB=<gib> or "
        "FREETOKEN_HOST_RAM_FRACTION=<0-1> if you accept the memory pressure;\n"
        "  - add host RAM."
    )
...
def load_expert_banks(
    ...
    * **Slow path** (the original checkpoint): auto-pick **parallel** (the common parallel chunked
    ...
    _check_host_ram_budget(model_path, dummy)
