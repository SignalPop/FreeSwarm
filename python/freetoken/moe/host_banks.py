    ...
    def release(self) -> None:
        ...
        if hasattr(self._buf, "madvise") and hasattr(mmap, "MADV_DONTNEED"):
            try:
                ...
                pass
        # Windows has no madvise, so without this a "released" bank stayed fully resident
        # and the converter's whole point -- one layer live at a time -- did not hold.
        from freetoken.utils import winio

        winio.discard(self._buf)
...
def read_file_into(buf: memoryview | mmap.mmap, path: str, *, workers: int = 8,
    ...
    if _sys.platform == "win32":
        # Windows: no O_DIRECT/preadv/posix_fadvise. FILE_FLAG_NO_BUFFERING is the O_DIRECT
        # equivalent and is what keeps a multi-GiB bank fill out of the system file cache;
        # buffered sequential read is the fallback when it is unusable.
        from freetoken.utils import winio
